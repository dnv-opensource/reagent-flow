...
{:deps  {org.clojure/clojurescript        {:mvn/version "1.11.51"}
 ...
 {:watch {:extra-deps {thheller/shadow-cljs {:mvn/version "2.20.20"}
  ...
  :build {:extra-deps {thheller/shadow-cljs {:mvn/version "2.20.20"}
                       binaryage/devtools   {:mvn/version "1.0.6"}}
...
(defn- flowjs->clj [o]
  (let [obj (js->clj o :keywordize-keys true)]
    (if (map? obj)
      (rename-keys obj (zipmap (keys obj) (map ->snake-case (keys obj))))
      obj)))
...
#+begin_src clojure :tangle babel/src/deps.cljs :noweb yes
{:npm-deps     {"reactflow" "<<version>>"}
 :npm-dev-deps {"create-react-class" "^15.7.0"
	        "react"              "^17.0.2"
	        "react-dom"          "^17.0.2"}}
#+end_src
...
#+begin_src clojure :tangle babel/package.clj
(ns package
  (:require
   [clojure.string :as str]
...
(defn jar [_]
  (let [file "reagent-flow.jar"]
    (b/uber {:class-dir "target/class"
             :uber-file file
             :basis     (b/create-basis {:project "deps.edn"})})
    (println (str file " created!"))))
#+end_src
